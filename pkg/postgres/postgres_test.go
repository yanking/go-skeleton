...
import (
	...
	"github.com/yanking/go-skeleton/pkg/postgres"
...
func dsnOrSkip(t *testing.T) string {
	...
	if dsn == "" {
		t.Skip("未设置 POSTGRES_DSN，跳过需要真实 PostgreSQL 的用例")
	}
	return dsn
}
...
func TestMustNewPanicsOnInvalidConfig(t *testing.T) {
	tests := []struct {
		...
		{name: "DSN 为空", cfg: postgres.Config{}},
		{name: "DSN 语法不对", cfg: postgres.Config{DSN: "postgres://user:pass@host:notaport/db"}},
		{
			// database/sql 的 Ping 完全不重试，连不上必须由本包重试到窗口耗尽再 panic，
			// 而不是「启动成功、第一个请求才 500」。
			name: "连不上时重试到超时",
			cfg: postgres.Config{
				DSN:            "postgres://postgres:secret@127.0.0.1:1/skeleton?sslmode=disable",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			postgres.MustNew(context.Background(), tt.cfg)
		})
	}
}

// Client 必须满足 pkg/app.Component，否则 cmd 无法把它注册进 app。
var _ app.Component = (*postgres.Client)(nil)
...
func TestConnectTimeoutIsRespected(t *testing.T) {
	...
	func() {
		...
		postgres.MustNew(context.Background(), postgres.Config{
			DSN:            "postgres://postgres:secret@127.0.0.1:1/skeleton?sslmode=disable",
...
func TestClientAgainstRealPostgres(t *testing.T) {
	dsn := dsnOrSkip(t)
	ctx := context.Background()
	c := postgres.MustNew(ctx, postgres.Config{DSN: dsn, MaxOpenConns: 7, Logger: discardLogger()})
...
func TestTelemetryProducesQuerySpan(t *testing.T) {
	...
	c := postgres.MustNew(ctx, postgres.Config{DSN: dsn, Telemetry: tel, Logger: discardLogger()})
	var one int
	if err := c.QueryRowContext(ctx, "SELECT 1").Scan(&one); err != nil {
		t.Fatalf("SELECT 1 失败: %v", err)
	}
